        ...
        builder: (context, AsyncSnapshot<QuerySnapshot<Map<String, dynamic>>> snapshotTrips) {
          if (snapshotTrips.hasData) {
            for (var item in snapshotTrips.data!.docs) {
              ...
              final matchingSlots = slots[date]!.where((s) => s.start == tripDate);
              final slot = matchingSlots.isNotEmpty ? matchingSlots.first : null;
              if (slot != null && slot.status != 2) {
                final slotIndex = slots[date]!.indexOf(slot);
